...
(function (global, factory) {
  ...
  const Split = function (idsOption, options) {
    ...
    let sizes = getOption(options, 'sizes') || ids.map(function () { return 100 / ids.length })

    // Standardize minSize to an array if it isn't already. This allows minSize
    // to be passed as a number.
    const minSize = getOption(options, 'minSize', 100)
    const minSizes = Array.isArray(minSize) ? minSize : ids.map(function () { return minSize })
    ...
    function trimToMin (sizesToTrim) {
      ...
      const pixelSizes = sizesToTrim.map(function (size, i) {
